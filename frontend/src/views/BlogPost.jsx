...
export default function BlogPost({ post: initialPost, slug: initialSlug, lang: propLang }) {
  const { lang, t } = useI18n(propLang);
  const post = initialPost || (initialSlug ? getBlogPostBySlug(initialSlug) : null);
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      {post.faqs && post.faqs.length > 0 && (
        <section className="my-10">
          <FAQSection lang={lang} customFaqs={post.faqs} title={`Frequently Asked Questions: ${post.category}`} />
        </section>
      )}
            ...
            {relatedPosts.map(rel => (
              ...
              >
                ...
                <h4 className="font-bold text-sm text-slate-900 dark:text-white">
                  <Link to={`/blog/${rel.slug}`} lang={lang} className="hover:underline">
                    {rel.title}
                  </Link>
                </h4>
